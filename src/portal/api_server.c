...
static bool verify_siwe_signature_address(const char *siwe_message, const char *siwe_signature, const char *expected_address) {
    ...
    return VerifySIWESignature((char*)siwe_message, (char*)siwe_signature, (char*)expected_address) == 1;
}
...
void handle_connect(cwist_http_request *req, cwist_http_response *res) {
    if (req->method != CWIST_HTTP_GET) {
        res->status_code = 405; /* CWIST_HTTP_METHOD_NOT_ALLOWED */
        cwist_sstring_assign(res->body, "{\"ok\": false, \"error\": {\"code\": \"invalid_request\", \"message\": \"method not allowed\"}}");
        cwist_http_header_add(&res->headers, "Content-Type", "application/json");
        return;
    }
    ...
    if (req->body && req->body->size > 0) {
        ...
        if (req_root) {
            ...
            if (id_obj && cJSON_IsObject(id_obj)) {
                ...
                char *msg = CreateSIWEMessage((char*)domain, (char*)addr, (char*)req_uri, (char*)nonce, (char*)"Register a portal lease", (char*)challenge_id, (char*)issued_at, (char*)expires_at, 1);
                if (!msg) {
...
void portillia_api_server_setup(cwist_app *app) {
    ...
    cwist_app_post(app, "/api/v2/unregister", handle_unregister);
