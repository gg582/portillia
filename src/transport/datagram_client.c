...
/* ---------- Frame encoding (Go types.EncodeDatagram / DecodeDatagram) ---------- */
...
static uint8_t *encode_datagram(uint32_t flow_id, const uint8_t *payload, size_t payload_len, size_t *out_len) {
    uint8_t varint_buf[5];
    size_t varint_len = encode_uvarint(varint_buf, flow_id);
    uint8_t *buf = (uint8_t *)portillia_gc_alloc(varint_len + payload_len);
    if (!buf) return NULL;
    memcpy(buf, varint_buf, varint_len);
    memcpy(buf + varint_len, payload, payload_len);
    *out_len = varint_len + payload_len;
    return buf;
}

static int decode_datagram(const uint8_t *data, size_t data_len, uint32_t *out_flow_id, const uint8_t **out_payload, size_t *out_payload_len) {
    size_t n = 0;
    if (decode_uvarint(data, data_len, out_flow_id, &n) != 0) {
        errno = EBADMSG;
        return -1;
    }
    *out_payload = data + n;
    *out_payload_len = data_len - n;
    return 0;
}
...
static void quic_recv_handler(void *user_data, const uint8_t *data, size_t len) {
    ...
    if (decode_datagram(data, len, &flow_id, &payload, &payload_len) != 0) return;
    portillia_datagram_frame_t frame = {0};
    frame.flow_id = flow_id;
    if (payload_len > 0 && payload) {
        frame.payload = (uint8_t *)portillia_gc_alloc(payload_len);
        if (frame.payload) {
...
int portillia_datagram_client_send(portillia_datagram_client_t *d, uint32_t flow_id, const uint8_t *payload, size_t payload_len) {
    ...
    /* Encode and push to outgoing queue (or directly to QUIC conn when wired). */
    size_t encoded_len = 0;
    uint8_t *encoded = encode_datagram(flow_id, payload, payload_len, &encoded_len);
